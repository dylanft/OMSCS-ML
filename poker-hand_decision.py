...
#  RUN EXPERIMENTS ON:  max_features
training_accuracy = []
validation_accuracy = []
test_accuracy = []
times = []
...
line3, = plt.plot(n_features, (test_accuracy), 'g', label="Testing Accuracy")
plt.xlabel('Max Features')
plt.ylabel('Accuracy')
plt.legend(loc='best')
plt.title('Max Features versus Accuracy (hand)')
fig.savefig('figures/ph/decision/poker-hand_decision_features.png')
plt.close(fig)
 
thefile = open('data/ph/decision/poker-hand_decision_features.txt', 'w')
...
plt.ylabel('Accuracy')
plt.legend(loc='best')
plt.title('Max Depth versus Accuracy (hand)')
fig.savefig('figures/ph/decision/poker-hand_decision_depth.png')
plt.close(fig)
 
thefile = open('data/ph/decision/poker-hand_decision_depth.txt', 'w')
...
plt.ylabel('Accuracy')
plt.legend(loc='best')
plt.title('Min Num of Samples / Split versus Accuracy (hand)')
fig.savefig('figures/ph/decision/poker-hand_decision_samples.png')
plt.close(fig)
 
thefile = open('data/ph/decision/poker-hand_decision_samples.txt', 'w')
...
#Use below to see Learning Curves
...
for n in training_size:
    ...
    for iteration in range(0, numIterations):
        ...
        hiddens = tuple(3 * [32])
        clf = tree.DecisionTreeClassifier(max_depth=7, min_samples_split=100, random_state=1)
...
fig.savefig('figures/ph/decision/poker-hand_decision_tsize.png')
plt.close(fig)
 
thefile = open('data/ph/decision/poker-hand_decision_tsize.txt', 'w')
